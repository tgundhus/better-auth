...
Groups support direct User members only. Nested Groups and members from another connection are rejected. A Group defaults to a maximum of 1,000 unique members. Set `groups.maxMembers` to a larger number, or `null` to remove the fixed membership cap. Membership changes remain atomic.
...
Keep identity and projection callbacks idempotent because a failed operation may be retried. A transaction adapter can advertise bounded batch work; when it does, projection callbacks for distinct users can run concurrently inside the same transaction. Do not coordinate different users through mutable process state, and keep every callback write on the supplied transaction. Decommissioning retains the connection's SCIM Users, Groups, and memberships.
