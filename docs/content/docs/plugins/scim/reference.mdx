...
Each `members[].value` must contain a SCIM User ID from the same connection. Members may be direct Users only. When `type` is present, it must equal `User` case-insensitively. Duplicate references are collapsed, and the default maximum is 1,000 unique members. Set `groups.maxMembers: null` to accept larger groups without a fixed member cap. Existing groups are always read completely, including when a deployment lowers its write limit.
...
The source list and decommissioned-connection lookups are fully paginated. They do not depend on the adapter's default page size, including when a user has sources from many connections. Profile ownership and session revocation use this complete source state.
...
When `bulk.jobs: true` is configured, this fork also provides durable job submission, authenticated status polling, and trusted queue, processing, and cleanup APIs. See [Durable SCIM Bulk jobs](/docs/plugins/scim/bulk-jobs) for the opt-in protocol, Lambda worker, idempotency, and retention contract. Standard SCIM requests retain their synchronous behavior.
...
* POST-based search, `/Me`, ETags, cursors, or sorting
